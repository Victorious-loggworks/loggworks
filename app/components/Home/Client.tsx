  ...
  return (
    <div className="px-20 container">
      <div className="flex flex-col items-center">
        <h2 className="text-primary-20 text-xl uppercase text-center font-semibold">
          FOR CLIENTS
        </h2>
        <p className="text-center text-gray-10 text-[40px] font-bold leading-[48px] capitalize mt-6">
          Discover, Compare, and Hire
        </p>
        <p className="text-xl mt-4 text-gray-30">
          Find Your Perfect Professional with Loggworks
        </p>

        <div className="w-full h-[674px] my-[72px] rounded-xl overflow-hidden">
          <video
            autoPlay
            loop
            muted
      ...
      <div>
        <div className="hidden lg:grid grid-cols-3 lg:grid-cols-3 gap-6 gap-y-6 mt-12">
          {data.map((image) => (
            <div
              key={image.id}
              className="flex flex-col gap-2 justify-center p-6 bg-gray-card rounded-2xl"
            >
              <div className="">
                <Image
                  src={image.src}
